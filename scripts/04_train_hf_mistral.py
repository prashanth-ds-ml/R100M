# scripts/04_train_hf_mistral.py
# Unified run folder + TRUE auto-resume + disk-safe (keeps ONLY latest & best)
# Mirrors the GPT/LLaMA trainer flow so runs/logs/checkpoints fit the same dashboard pipeline.
#
# Usage example:
...
#     --run_dir "C:\Users\prash\Projects\Rachana_GPT\runs\rachana_mistral_tiny_80m" `
...
from transformers import MistralConfig, MistralForCausalLM, get_cosine_schedule_with_warmup
...
@torch.no_grad()
def evaluate_loss(model: MistralForCausalLM, loader: DataLoader, device: str, use_amp: bool, max_batches: int) -> float:
    model.eval()
    losses = []
    it = iter(loader)
...
def load_latest_if_any(run_ckpt_dir: str, device: str) -> Optional[Tuple[str, Dict[str, Any], MistralForCausalLM]]:
    latest_dir = os.path.join(run_ckpt_dir, "latest")
    st_path = os.path.join(latest_dir, "trainer_state.pt")
    if not os.path.isfile(st_path):
        return None
    st = torch.load(st_path, map_location="cpu", weights_only=False)
    model = MistralForCausalLM.from_pretrained(latest_dir).to(device)
...
def main(cfg: TrainConfig):
    ...
    if resumed is not None:
        ...
        model = MistralForCausalLM(mistral_cfg).to(device)
        print("Starting fresh Mistral-style run.")
...
def parse_args():
    ...
    p.add_argument("--intermediate_size", type=int, default=0, help="0 => use Mistral-style derived default")
