# scripts/04_train_hf_hybrid.py
# Pragmatic hybrid trainer:
# - LLaMA/Mistral-family decoder-only stack
# - RoPE
# - RMSNorm
# - SwiGLU MLP
...
#
# This keeps the same run/log/checkpoint pipeline as the GPT/LLaMA/Mistral trainers
# so runs plug into the same dashboard and experiment layout.
#
...
@torch.no_grad()
def evaluate_loss(model, loader: DataLoader, device: str, use_amp: bool, max_batches: int) -> float:
    model.eval()
    losses = []
    it = iter(loader)
...
def load_latest_if_any(run_ckpt_dir: str, device: str) -> Optional[Tuple[str, Dict[str, Any], Any]]:
    latest_dir = os.path.join(run_ckpt_dir, "latest")
...
def build_hybrid_model(
    ...
    if cfg.backbone == "llama":
        hybrid_cfg = LlamaConfig(**common)
        model = LlamaForCausalLM(hybrid_cfg).to(device)
    elif cfg.backbone == "mistral":
        hybrid_cfg = MistralConfig(sliding_window=cfg.sliding_window, **common)
        model = MistralForCausalLM(hybrid_cfg).to(device)
    else:
        raise ValueError(f"Unsupported hybrid backbone: {cfg.backbone}")
...
def main(cfg: TrainConfig):
    ...
    if not os.path.exists(meta_path):
        save_json(meta_path, {
            ...
            "hybrid_notes": [
                ...
                ("sliding-window-support" if cfg.backbone == "mistral" else "llama-backbone"),
            ],
        })
    ...
    if resumed is not None:
        ...
        if cfg.backbone == "llama" and cfg.sliding_window != cfg.max_length:
            print("[note] llama backbone ignores sliding_window; using standard full attention.")
        model = build_hybrid_model(cfg, vocab_size, pad_id, bos_id, eos_id, device)
        print(f"Starting fresh hybrid run with {cfg.backbone} backbone.")
...
def parse_args():
    ...
    p.add_argument("--backbone", type=str, default="mistral", choices=["llama", "mistral"])
    ...
    p.add_argument("--n_kv_head", type=int, default=None, help="Defaults to grouped-query style max(1, n_head//4)")
    p.add_argument("--intermediate_size", type=int, default=0, help="0 => use hybrid derived default")
    p.add_argument("--rms_norm_eps", type=float, default=1e-6)
    p.add_argument("--rope_theta", type=float, default=10000.0)
    p.add_argument("--sliding_window", type=int, default=0, help="0 => use max_length")
...
if __name__ == "__main__":
    a = parse_args()
    n_kv_head = max(1, a.n_head // 4) if a.n_kv_head is None else a.n_kv_head
    intermediate_size = resolve_intermediate_size(a.n_embd, a.intermediate_size)
    sliding_window = a.max_length if a.sliding_window <= 0 else a.sliding_window
    ...
    cfg = TrainConfig(
        ...
        attention_dropout=a.attention_dropout,
