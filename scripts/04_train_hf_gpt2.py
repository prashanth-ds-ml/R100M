# scripts/04_train_hf_gpt2.py
# Unified run folder + TRUE auto-resume + disk-safe (keeps ONLY latest & best) + richer metrics (+ optional val)
#
# Usage (recommended unified folder):
...
#     --run_dir "C:\Users\prash\Projects\Rachana_GPT\runs\rachana_80M_len512" `
#     --max_length 512 --stride 512 `
#     --n_layer 10 --n_head 12 --n_embd 768 `
...
#     --val_fraction 0.005
#
# Resume is automatic: if run_dir/checkpoints/latest exists => resumes.
# Disk stays bounded: checkpoints/latest and checkpoints/best only.
#
...
class TrainConfig:
    ...
    weight_decay: float
    warmup_steps: int
    max_steps: int                   # micro-steps (loader iterations)
    ...
    val_fraction: float              # 0 disables validation
    eval_batches: int                # how many batches to evaluate (caps time)
...
def safe_exp(x: float, cap: float = 50.0) -> float:
    # cap to avoid overflow early in training
    x = float(min(x, cap))
    return float(np.exp(x))

def atomic_replace_dir(src_dir: str, dst_dir: str):
    """
    Replace dst_dir with src_dir contents atomically-ish on Windows:
...
def load_trainer_state(ckpt_dir: str) -> Dict[str, Any]:
    ...
    return torch.load(st_path, map_location="cpu",weights_only=False)


# -------------------------
# Dataset (memmap-backed)
# -------------------------
...
# -------------------------
@torch.no_grad()
def evaluate_loss(model: GPT2LMHeadModel, loader: DataLoader, device: str, use_amp: bool, max_batches: int) -> float:
    model.eval()
    losses = []
    it = iter(loader)
...
# -------------------------
# Checkpoint IO (latest/best only)
# -------------------------
...
def load_latest_if_any(run_ckpt_dir: str, device: str) -> Optional[Tuple[str, Dict[str, Any], GPT2LMHeadModel]]:
    latest_dir = os.path.join(run_ckpt_dir, "latest")
    st_path = os.path.join(latest_dir, "trainer_state.pt")
    if not os.path.isfile(st_path):
        return None
    st = torch.load(st_path, map_location="cpu",weights_only=False)
    model = GPT2LMHeadModel.from_pretrained(latest_dir).to(device)
...
def main(cfg: TrainConfig):
    ...
    if cfg.val_fraction and cfg.val_fraction > 0.0:
        ...
        # ensure train slice large enough
    ...
    # Optimizer/Scheduler (MUST be created after final model instance)
    ...
    # load opt/sched state after creation
    if resumed is not None:
        st = resumed[1]
    ...
    while micro_step < cfg.max_steps:
        ...
        try:
            ...
            if accum_count >= cfg.grad_accum:
                ...
                # update tokens_total on optimizer update
                tokens_total += tokens_in_update

                # compute train_loss (true) for logging
                train_loss_true = float(loss.detach().cpu()) * cfg.grad_accum

            # ---------- EVAL (optional, per update) ----------
...
def parse_args():
    ...
    # IMPORTANT: unified folder. No timestamps unless YOU want to.
...
if __name__ == "__main__":
    ...
    # Save cfg snapshot once (do not overwrite if exists)
